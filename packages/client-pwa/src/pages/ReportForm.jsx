...
function ReportForm() {
  ...
  // --- Track Reports by Mobile ---
  const handleTrackReports = async () => {
    setTrackingError("");
    setTrackedReports([]);

    if (!trackMobile) {
      setTrackingError("Please enter your mobile number.");
      return;
    }

    try {
      const response = await fetch(
        `http://localhost:5000/api/issues/by-mobile/${trackMobile}`
      );
      ...
      if (data.length === 0) {
        setTrackingError("No reports found for this number.");
      } else {
        setTrackedReports(data);
      }
    } catch (err) {
      console.error("Tracking error:", err);
      setTrackingError("Error fetching reports. Please try again.");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
      ...
      {/* --- Track Reports --- */}
      <div className="w-full max-w-lg mx-auto bg-white rounded-xl shadow-lg p-6 md:p-8 mt-8">
        <h2 className="text-xl font-bold text-gray-800 mb-4">Track My Reports</h2>
        ...
        {trackingError && (
          <p className="text-red-600 text-sm mb-4">{trackingError}</p>
        )}
